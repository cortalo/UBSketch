...
template<int memory_in_bytes, int filter_size, int d>
class ASketch
{
private:
	static constexpr int w = (memory_in_bytes - filter_size * 12) * 8 / BITS_PER_BUCKET / d;
...
public:
	...
	void insert(string string_key, int f)
	{
		uint32_t key = *(uint32_t*)string_key.c_str();
